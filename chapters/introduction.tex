...
These programs perform a wide range of functions necessary for life, including metabolism, growth, reproduction, response to stimuli, and adaptation to the environment.
...
This thesis is largely concerned with the development and application of such machine-learning based computational tools to problems tackled by single-cell profiling technologies.
...
At the core of each cell's identity is the \emph{genetic code}, a long sequence encoded biologically as DNA molecules.
...
Thus, not just the content of the genetic code but also how the genetic code is processed plays a critical role in determining the cell.
...
On a high level, genes, particular sections of the genetic code stored in the form of DNA, are transcribed into a corresponding RNA molecule, which is then translated into some specific protein.
...
\paragraph{DNA} The genetic code is stored in the form of DNA (deoxyribose nucleic acid) molecules.
...
In a process called \emph{translation}, RNA strands are read to create specific proteins.
...
Gene expression is also used to understand what processes the cell activates or deactivates due to changes in its environment.
...
Proteins, however, have complicated 3D shapes that are fundamentally tied to their functions.
...
As a result of this complexity, proteins have high specificity for ligand binding and are usually only able to interact with a few ligands.
...
Pathways govern fundamental cellular decisions, such as dictating how and when to grow, divide, differentiate, die, etc.
...
A useful abstraction is the distinction between an organism's \emph{genotype}, the full set of information contained in its genetic code, and its \emph{phenotype}, its observable characteristics.
...
Despite this, cancer has remained an active research environment, challenging researchers to understand and control complex and nuanced cellular behaviors that can oftentimes be specific to the tumor.
...
A major complication in the treatment of cancer lies in the myriad mechanisms cells can use to escape the natural regulatory processes that prevent unchecked growth.
...
In healthy cells, the signals that govern these processes are well regulated so that normal functions are maintained.
...
Another class of driver mutations occurs in so-called "tumor suppressor genes."
...
\emph{somatic} mutations that are acquired throughout one's lifetime, e.g., by smoking, exposure to UV radiation, or simply random chance \cite{hanahan2011}.
A complication towards robust cancer treatments stems from the fact that individuals will have unique mutation combinations, and the development of "one-size-fits-all" treatments is difficult or impossible.
Furthermore, while driver mutations appear in some subset of genes, they can affect different subunits and/or have different effects on the protein's downstream behavior \cite{smiech2020}.
...
As a result, the genetics of cancer cells can exhibit a branching process within tumor clones, resulting in what are known as tumor subclones.
...
developing a sweeping, generalized treatment for cancer is difficult.
...
immunotherapy, which bolsters the host immune system to identify and eliminate cancer cells,
and chemotherapy, which targets and attacks the cancer cells themselves \cite{hanahan2011}.
...
a monumental scientific achievement that laid the groundwork for the development of "next-gen" sequencing technology.
The human genome contains billions of nucleotides, so measuring this sequence end-to-end is infeasible.
Next-gen sequencing technologies isolate and measure the nucleotide sequence of small fragments called reads instead.
...
These technologies offer cheaper but low-resolution profiles, as the information from the individual is grouped together.
...
This approach is still quite limited as it relies on cell states that are conserved across many individuals in the population, and we have seen,
...
that has revolutionized our capability to study cellular behaviors, especially in the context of heterogeneity \cite{moty2014}.
These technologies are capable of mechanically isolating individual cells and then sequencing their expressed RNA.
...
For reference, humans have 20k protein-coding genes.
...
Popular computational strategies utilized in the analysis of scRNA-seq data are explored in section \ref{sec:scrna-seq}.
...
These proteins are called "markers" as they "mark" the activity level of their respective pathway.
...
The number of features a label-based profiling methods are limited by the facts that i) the modular groups need to be designed to uniquely bind to specific individual proteins and ii) the signals the labels emit are easy to measure because they have wide "bandwidths" in their obserble spaces
...
CyTOF, which communicates signals in a chemical space, and 4i, which communicates signals in a visual space.
...
Thus, the atomic cloud is observed as a composite signal over the mass-to-charge ratios of all atoms within.
...
that is then measured with an extremely high-resolution camera capable of observing sub-cellular features.
...
4i describes a novel iterative washing and staining protocol that allows for the use of differently labeled color channels.
...
and 2) that this analysis can inform and improve traditional clinical decision-making.
...
\begin{figure}
  ...
  \end{center}
  \caption{Overview of the Tumor Profiler project, adopted from \cite{irmisch2020}. The project is split into two stages: a fast diagnostic loop, which performs multi-modal single-cell profiling on biopsies of incoming cancer patients to supplement traditional diagnostic strategies, and an exploratory science loop, which performs cohort-wide analysis on the collected samples.
...
this report is further distilled by a "pre-tumor" board and finally by the tumor board, where treatment decisions are made.
For each sample, the target turnaround time for collection, profiling, and analysis is two weeks.

\section{Representation learning of scRNA-seq data} \label{sec:scrna-seq}
Raw readouts of single-cell profiling technologies often have statistical properties that are difficult for computational tools to handle.
...
Additionally, there are added challenges due to batch effects, technical variations that arise from uncontrollable differences in experimental conditions that can result in differences in the measured gene expression levels between different runs, or batches, of the same experiment.
...
While this normalization does not directly address the variation and mechanisms described above, it has been shown to perform surprisingly well \cite{ahlmann-eltze2023}.
...
Therefore, the library size is typically normalized such that the total number of counts within an individual cell is constant.
...
Even after normalization, the sparsity and high-dimensionality of scRNA-seq data still remain challenges.
...
Oftentimes, this step can reduce the dimensionality from the tens of thousands to the much more manageable hundreds to thousands \cite{satija2015,zheng2017,stuart2019}.
...
Let $x \in \mathbb{R}^D$ be a data point in the high-dimensional feature space and let its corresponding representation in the low-dimensional space be $z \in \mathbb{R}^d$.
...
The KL term in equation \ref{eq:vae-loss} is often regarded as a regularization term that enforces the "compactness" of the latent space.
...
So, while we would ideally like to make multiple observations of an individual cell, for example, with different profiling technologies or at different points in time, we are typically limited to making only one observation.
...
The challenge of methods development is to reconstruct the corresponding cellular states by \emph{alignment} of these multiple observations.
...
For instance, the relationship between genes and proteins is known to not be 1-to-1 \cite{edfors2016}.
...
Cellular responses to perturbations can be considered a temporal process.
